    ...
    public function exportExcel(Request $request)
    {
        ...
        if (!is_null($based)) {
            ...
            if ($based == 'na') {
                ...
                if ($cndn == 'approved' || $cndn == 'pending' || $cndn == 'declined') {

                    $cndn = ($cndn == 'approved') ? 'approve' : ($cndn == 'pending' ? 'pending' : ($cndn == 'declined' ? 'decline' : ''));


                    if ($based == 'location' || $based == 'branch') {

                        $users = $users->where('status', $cndn)->WhereIn('branch', $ev);
                    }

                    if ($based == 'gender') {

                        $users = $users->where('status', $cndn)->withWhereHas('personalInformation', function ($q) use ($ev) {
